    ...
    if nplot gt 0 then begin
       ...
       if onedcount gt 0 then begin
          ...
          if count gt 0 then begin
             ...
             if n_elements(newlabel) ne 0 then item_name[0:((count<n_elements(newlabel))-1)] = ' '+newlabel[0:((count<n_elements(newlabel))-1)]
...
;return red if there is only green plots, otherwise return green
function dm_plot::zoomboxcolor
    nplot = n_elements(*self.plotInfo)
    if nplot ne 0 then begin
       tmp = where(((*self.plotInfo).color[0] ne 0) or ((*self.plotInfo).color[1] ne 255) or ((*self.plotInfo).color[2] ne 0),cnt)
       if cnt eq 0 then return,self->getcolor('red')
    endif
...
;fontsize keywords:
;   fsize:      an integer used for all font sizes
;   axisfsize:  an integer used for the font size of all axis titles
;   cbarfsize:  an integer used for the font size of the color intensity bar title
;   cursfsize:  an integer used for the font size of the cursor position text
;   ctxtfsize:  an integer used for the font size of the coner text
;   legdfsize:  an integer used for the font size of the legend  
;   tickfsize:  an integer used for tht font size of all axis tick text
;   titlfsize:  an integer used for the font size of the plot title
function dm_plot::Init,xdat,ydat,zdat,idat,xerr=xerr,yerr=yerr,zerr=zerr,ierr=ierr,addarrow=addarrow,arrowangle=arrowangle,arrowlength=arrowlength,axisfsize=axisfsize,axisthick=axisthick,$
    background=background,cbarfsize=cbarfsize,cbarpos=cbarpos,coldefine=coldefine,coltable=coltable,color=color,compound=compound,contourplot=contourplot,cornertxt=cornertxt,$
    ctxtfsize=ctxtfsize,cursfsize=cursfsize,cutcolor=cutcolor,draw_motion_events=draw_motion_events,extragap=extragap,font=font,fsize=fsize,gamma=gamma,gridthick=gridthick,$
    gridlinestyle=gridlinestyle,gridontop=gridontop,group_leader=group_leader,hideaxes=hideaxes,hideintbar=hideintbar,hidelegend=hidelegend,isolatin1=isolatin1,isotropic=isotropic,$
    iso3dtype=iso3dtype,irange=irange,ilog=ilog,ititle=ititle,interpolate=interpolate,keep=keep,layer=layer,legdcolumns=legdcolumns,legdfsize=legdfsize,legdgap=legdgap,$
    legdbordergap=legdbordergap,legdglyphwidth=legdglyphwidth,legdoutlinethick=legdoutlinethick,legdpos=legdpos,legdshowfill=legdshowfill,legdshowoutline=legdshowoutline,legend=legend,$
    ...
    showygrid=showygrid,showzgrid=showzgrid,surfplot=surfplot,symsize=symsize,thick=thick,tickfsize=tickfsize,title=title,titlfsize=titlfsize,usepolygon=usepolygon,vector=vector,$
    volclip=volclip,vt_col=vt_col,vtrm_pt=vtrm_pt,widgetbase=widgetbase,wtitle=wtitle,xinfo=xinfo,xlog=xlog,xrange=xrange,xtitle=xtitle,xsize=xsize,yinfo=yinfo,ysize=ysize,ylog=ylog,$
    ...
    self.info   = ptr_new(['dm_plot: IDL '+!version.release+' Object Graphics for 1D, 2D, and 3D Plots','','Yiming Qiu (yiming.qiu@nist.gov)  12/2020'])
