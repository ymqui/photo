...
;fontsize keywords (corresponding font name keyword in praentheisi):
...
function dm_plot::Init,xdat,ydat,zdat,idat,xerr=xerr,yerr=yerr,zerr=zerr,ierr=ierr,addarrow=addarrow,arrowangle=arrowangle,arrowlength=arrowlength,axisfsize=axisfsize,axisfont=axisfont,$
    ...
    if n_elements(tickfont) eq 1 then self.font[0]->setproperty,name=tickfont    
    if n_elements(axisfont) eq 1 then for i=1,3 do self.font[i]->setproperty,name=axisfont
    if n_elements(titlfont) eq 1 then self.font[4]->setproperty,name=titlfont
    if n_elements(legdfont) eq 1 then self.font[5]->setproperty,name=legdfont
    if n_elements(cursfont) eq 1 then self.font[6]->setproperty,name=cursfont
    if n_elements(ctxtfont) eq 1 then self.font[7]->setproperty,name=ctxtfont
    if n_elements(cbarfont) eq 1 then self.font[8]->setproperty,name=cbarfont
    ...
    self.info   = ptr_new(['dm_plot: IDL '+!version.release+' Object Graphics for 1D, 2D, and 3D Plots','','Yiming Qiu (yiming.qiu@nist.gov)  8/2022'])
