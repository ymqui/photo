pro find_spec,name
    ...
    for i=0,n_elements(name)-1 do begin
        print,reform_name(name[i])+': '+string(9b)+latin[i]+strjoin(strarr(1>(30-strlen(latin[i]))),' ')+string(9b),chinese[i]+string(9b),line[i],existed[i]?' (already in the list)':' (after '+after[i]+')'
    endfor
end
...
function get_filename,excel=excel,chinese=chinese,ibn=ibn,order=order,birdname=birdname,namecomp=namecomp,locs=locs,movie=movie,tmpdir=tmpdir,totbird=totbird,country=country
    ...
    if keyword_set(excel)    then return,'C:\Users\qiuym\Documents\Yiming\tmp\master_ioc_list_v12.2.csv'
    if keyword_set(chinese)  then return,'C:\Users\qiuym\Documents\Yiming\tmp\Multiling_IOC_V12.2.csv'
...
pro print_birds
    read_ibn,line=line
    ind = where(stregex(line,'"njscmm",',/boolean,/fold_case),cnt)
    line = line[ind]
    for i=0,cnt-1 do begin
        tmp = strsplit(line[i],'"',/extract)
        print,i+1,'. ',tmp[5]
    endfor
end
