...
pro find_spec,name,locid=locid,pid=pid,ebird=ebird
    ...
    for i=0,n_elements(name)-1 do begin
        ...
        if (~existed[i]) and (strlen(chinese[i]) gt 0) then begin
           print,' '
           print,'//  ["'+year+'-00-00T00:00","'+reform_name(family[i],/family)+'","'+reform_name(name[i])+'","'+chinese[i]+'","'+latin[i]+'",'+pid+'],["'+locid+'","'+ebird+'"],"'+reform_name(name[i],/ebird)+'"],'
           print,' '
        endif
    endfor
...
function get_filename,excel=excel,chinese=chinese,ibn=ibn,order=order,birdname=birdname,namecomp=namecomp,locs=locs,movie=movie,tmpdir=tmpdir,totbird=totbird,country=country,extra=extra
    ...
    if keyword_set(chinese)  then return,'C:\Users\qiuym\Documents\Yiming\tmp\Multiling_IOC_14.1.csv'
    ...
    if keyword_set(movie)    then return,'C:\Users\qiuym\Desktop\lifer'
...
pro read_ibn,bird=bird,latin=latin,line=line,all=all,year=year,family=family,chinese=chinese,stat=stat,extra=extra
    ...
    if arg_present(year) or keyword_set(stat) then begin
    ...
    if keyword_set(stat) then begin
       ...
       read_order,family=tmp1,order=tmp2,listorder=tmp3
       tmp1 = strmid(tmp1,0,8)
       tmp4 = strarr(n_elements(tmp))
       for i=0,n_elements(tmp)-1 do tmp4[i] = tmp3[where(tmp1 eq strtrim(tmp[i]))]
       print,dm_to_string(cnt)+'/'+dm_to_string(count)+' lifers, '+dm_to_string(n_elements(tmp))+'/'+$
             dm_to_string(n_elements(tmp1))+' families, '+dm_to_string(n_elements(uniq(tmp4,sort(tmp4))))+'/'+$
             dm_to_string(n_elements(tmp2))+' orders.'
    endif
end
...
pro read_order,family=family,order=order,line=line,all=all,listorder=listorder
    if n_elements(list) ne 0 then tmp = temporary(list)
    ...
    for i=ind0[0],ind1[0]-1 do begin
        ...
        while stregex(tmp1,'(iformes|idae)',/fold_case,/boolean) do begin ;in case multiple family in one line
              ...
              if stregex(tmp[0],'iformes',/boolean,/fold_case) then begin
                 order = [order,tmp[1]]
              endif else begin
                 family = [family,tmp[1]]
    ...
    if n_elements(listorder) gt 1 then listorder = listorder[1:*]
end

pro check_file
    current = systime(/sec)
    read_ibn,bird=bird,line=line,latin=latin,family=family,chinese=chinese,/stat
    if n_elements(bird) eq 0 then return
    print,'please wait...'
    ind0 = find_spec_pos(bird)
    tmp  = where(ind0 gt 0,cnt)
    ind1 = ind0[tmp]
    ind2 = sort(ind1)
    newline = line
    newline[tmp] = line[tmp[ind2]]
    for i=0,cnt-1 do begin
        if i ne ind2[i] then begin
           if n_elements(pair) eq 0 then pair = [i<ind2[i],i>ind2[i]] else begin
              notin = 1
              for j=0,n_elements(pair[0,*])-1 do begin
                  if ((i ge pair[0,j]) and (i le pair[1,j])) or ((ind2[i] ge pair[0,j]) and (ind2[i] le pair[1,j])) then begin
                     pair[0,j] = pair[0,j]<(i<ind2[i])
                     pair[1,j] = pair[1,j]>(i>ind2[i])
                     notin = 0
                     break
                  endif
              endfor
              if notin then pair = [[pair],[i<ind2[i],i>ind2[i]]]
           endelse
        endif
    endfor
    allgood = n_elements(pair) eq 0
    for i=0,n_elements(pair)/2-1 do begin
        for j=pair[0,i],pair[1,i] do begin
            print,newline[j]
        endfor
        print,''
    endfor
...
;    endif
    ...
    for i=0,n_elements(year)-1 do begin
        if keyword_set(updown) then begin
           if year[i] gt 2011 then factor[0] = 1.2-((year[i]) mod 2)*3.5
           if year[i] gt 2015 then factor[1] = 1.2-((year[i]+1) mod 2)*3.5
        endif
        if i eq 0 then begin
           ...
           pobj->add_text,dm_to_string(cuml[i]),year[i],cuml[i]+gap*factor[0],normalize=0,orientation=0,vertical_alignment=0,otext=otext1,fontsize=fsize
        endelse
        pobj->add_text,dm_to_string(cnts[i]),year[i],cnts[i]+gap*factor[1],normalize=0,orientation=0,vertical_alignment=0,otext=otext2,fontsize=fsize
    ...
    ;;chinese version
    pobj->updatelegend,newlabel=['hi,there,a','hi']
    image1 = pobj->getimagedata()
    image1[*,offset[0]:(offset[0]+dim[1]-1),offset[1]:(offset[1]+dim[2]-1)] = image2
    ;legend location
