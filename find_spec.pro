pro find_spec,name
    ...
    line = find_spec_pos(name,after=after,latin=latin,existed=existed)
    ...
    for i=0,n_elements(name)-1 do begin
        print,reform_name(name[i])+': '+string(9b)+latin[i]+strjoin(strarr(1>(30-strlen(latin[i]))),' ')+string(9b),line[i],existed[i]?' (already in the list)':' (after '+after[i]+')'
    endfor
end
...
function find_spec_pos,name,print=print,after=after,latin=latin,existed=existed
    if n_elements(name) eq 0 then begin
...
pro plot_lifer,wait=wait,movie=movie,chinese=chinese,image2=image2,pobj=pobj
    ...
    pobj  = obj_new('dm_plot',xtitl=xtit,ytit=ytit,cornertxt='LaoQ \copyright '+dm_to_string(min(year))+'-'+dm_to_string(max(year)),$
            /legdshowfill,/showygrid,axisfsize=16,ctxtfsize=10,ctxtfont='DeJaVuSans Italic',wtitle='Lifer Count',xsize=900,ysize=520)
