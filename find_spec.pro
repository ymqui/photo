pro find_spec,name,locid=locid,pid=pid
    ...
    for i=0,n_elements(name)-1 do begin
        ...
        if existed[i] eq 0 then begin
           check_hanzi,chinese[i],py_wch=py_wch,py_mis=py_mis
           print,' '
           print,'//birds[j++] = new Bird("'+year+'-00-00T00:00","'+reform_name(family[i],/family)+'","'+reform_name(name[i])+'","'+chinese[i]+'","'+latin[i]+'",'+mid+'),["'+locid+'",""],"'+reform_name(name[i],/ebird)+'");'
           print,' '
        endif
    endfor
...
pro check_chinese,allgood=allgood,ibn=ibn
    ...
    for i=0,n_elements(birds3)-1 do begin
        ...
        if cnt eq 0 then begin
           print,"Can't find the Chinese name for "+birds3[i]+'.'
           continue
        endif else begin
           if chinese1[ind[0]] ne chinese3[i] then begin
              print,birds3[i],'  ',chinese3[i],'  ',chinese1[ind[0]]
              j = j+1
           endif
        endelse
    endfor
...
;    ind = where(stregex(line,'^  birds\[j\+\+\].*"23\-01\-.*"(papipe|pametr|pacasc|papuru|arb)",.*',/boolean,/fold_case),cnt)
    ind = where(stregex(line,'m_id\(""\),\[',/boolean),cnt)
    line1 = line[ind]
    for i=0,cnt-1 do begin
        if ~stregex(line1[i],'arb',/boolean) then print,line1[i]       
...
pro find_bird
    ...
    pattern = '[0-9]{2,}\);$'
    for i=0,n_elements(line)-1 do begin
        if stregex(line[i],pattern,/boolean) then print,line[i]
    endfor
    print,'Done'
end
