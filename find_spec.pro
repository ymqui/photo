...
function get_filename,excel=excel,chinese=chinese,ibn=ibn,order=order,birdname=birdname,namecomp=namecomp,locs=locs,movie=movie,tmpdir=tmpdir,totbird=totbird,country=country,extra=extra
  ...
  if keyword_set(ibn)      then return,'C:\Users\Maomao\Desktop\index_birds_names.js'
  if keyword_set(excel)    then return,'C:\Users\Maomao\Desktop\Yiming\tmp\master_ioc_list_v14.1.csv'
  if keyword_set(chinese)  then return,'C:\Users\Maomao\Desktop\Yiming\tmp\Multiling_IOC_14.1.csv'
  if keyword_set(order)    then return,'https://ymqui.github.io/photo/info/index_birds_orders.js'
  ;if keyword_set(order)    then return,'C:\Users\Maomao\Desktop\index_birds_orders.js'
  if keyword_set(namecomp) then return,'C:\Users\Maomao\Desktop\Yiming\tmp\ibn_namecompare.txt'
  if keyword_set(movie)    then return,'C:\Users\Maomao\Desktop\lifer.mp4'
  ...
  if keyword_set(tmpdir)   then return,'C:\Users\Maomao\Desktop\Yiming\tmp\'
  if keyword_set(extra)    then return,'https://ymqui.github.io/photo/info/ibn_extra.js'
  ;if keyword_set(extra)    then return,'C:\Users\Maomao\Desktop\Yiming\tmp\ibn_extra.js'
...
pro check_file
    ...
    print,'please wait...'
    ind0 = find_spec_pos(bird)  & help,ind0,bird ,line 
        ...
        if i ne ind2[i] then begin
           if n_elements(pair) eq 0 then pair = [i<ind2[i],i>ind2[i]] else begin
              ...
              for j=0,n_elements(pair[0,*])-1 do begin
                  if (((i<ind2[i]) ge pair[0,j]) and ((i<ind2[i]) le pair[1,j])) or (((i>ind2[i]) ge pair[0,j]) and ((i>ind2[i]) le pair[1,j])) then begin
