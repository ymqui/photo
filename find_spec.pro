...
function get_filename,excel=excel,ibn=ibn,order=order,birdname=birdname,namecomp=namecomp,locs=locs,movie=movie,tmpdir=tmpdir,totbird=totbird,country=country
    ...
    if keyword_set(excel)    then return,'C:\Users\qiuym\Documents\Yiming\tmp\master_ioc_list_v11.2.csv'
...
pro plot_lifer,wait=wait,movie=movie,chinese=chinese,image2=image2
    ...
    dm_step_bin,1.0,year,ydat=cnts,/avgsum  ;aggregate based on year
    ...
    pobj->setproperty,yran=yran+[-0.1,0.15]*(yran[1]-yran[0]),xran=xran+[-0.08,0.08]*(xran[1]-xran[0]),ytickminor=0,/nodraw,extragap=[-0.02,0,0,0]
    ...
    for i=0,n_elements(year)-1 do begin
        if i eq 0 then begin
           ...
           pobj->add_text,dm_to_string(cuml[i]),year[i],cuml[i]+gap,normalize=0,orientation=0,vertical_alignment=0,otext=otext1,fontsize=12
        endelse
        pobj->add_text,dm_to_string(cnts[i]),year[i],cnts[i]+gap*(i eq 1?(-2.5):1),normalize=0,orientation=0,vertical_alignment=0,otext=otext2,fontsize=12
    ...
    title =(['Total','!Z(9E1F,79CD,603B,6570)'])[keyword_set(chinese)]+'='+dm_to_string(total(cnts))+', '+ (['Annual Avg','!Z(5E74,5747)'])[keyword_set(chinese)]+'='+dm_to_string(total(cnts)/(max(year)-min(year)+1.0),res=1)
    pobj->setproperty,title=title
